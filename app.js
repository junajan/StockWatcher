/**
 * This code written in node.js will every day in workweek send email/sms with rates of  
 * difference between requested SMA (for given indexes) and closePrices
 *
 * For instalation:
 *  - get node.js & npm
 *  - npm install
 ...
 * SPY:100=3.76;SPY:200=6.12;DIA:100=4.35;DIA:200=5.93;VTI:100=3.34;VTI:200=5.33
...
function serializeResult(res) {
	...
	res.forEach(function(arr) {
		arr.forEach(function(resItem) {
			out.push(resItem.ticker+":"+resItem.len+"="+getPercentChange(resItem.lastClose, resItem.sma));
		});
	});
	return out.join(";");
}
...
function processResult(err, res) {
	...
	var resSerialized = serializeResult(res);
	console.timeLog("Sending email..".yellow, resSerialized);
	sendEmail(resSerialized);
